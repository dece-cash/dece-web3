...
var opArgs = function (abi, params,rand,dece,dy,callback) {
    if (callback){
        ...
        if (types&&types[0]){
            coder.opParams(types[0], params,rand,dece,dy,callback);
        }else {
            coder.opParams([], [],rand,dece,dy,callback)
        }
        ...
        return abi.filter(function (json) {
            ...
            return coder.opParams(types, params,rand,dece,dy);
        })[0] || coder.opParams([], [],rand,dece,dy);
    }
...
var addEventsToContract = function (contract) {
    ...
    var All = new AllEvents(contract._dece, events, contract.address);
    All.attachToContract(contract);

    events.map(function (json) {
        return new SolidityEvent(contract._dece, json, contract.address);
    }).forEach(function (e) {
...
var checkForContractAddress = function(contract, callback){
    ...
    // wait for receipt
    var filter = contract._dece.filter('latest', function(e){
        if (!e && !callbackFired) {
            ...
            if (count > 50) {
                ...
                contract._dece.getTransactionReceipt(contract.transactionHash, function(e, receipt){
                    if(receipt && receipt.blockHash && !callbackFired) {

                        contract._dece.getCode(receipt.contractAddress, function(e, code){
 ...
 * @param {Array} abi
 */
var ContractFactory = function (dece, abi) {
    this.dece = dece;
    this.abi = abi;
    ...
    this.new = function () {
        ...
        var contract = new Contract(this.dece, this.abi);
        ...
        var rand ="0x";
        var argsResult = opArgs(this.abi,args,rand,this.dece,false);
        ...
        if (callback) {
            ...
            this.dece.sendTransaction(options, function (err, hash) {
                if (err) {
                    callback(err);
                } else {
            ...
            var hash = this.dece.sendTransaction(options);
            // add the transaction hash
            contract.transactionHash = hash;
...
ContractFactory.prototype.at = function (address, callback) {
    var contract = new Contract(this.dece, this.abi, address);
...
ContractFactory.prototype.getData = function () {
    ...
    if (callback){
        ...
        opArgs(this.abi,args,rand,self.dece,false,cb);

    }else{

        argsResult = opArgs(self.abi,args,rand,self.dece,false);
        args = argsResult.params;
        rand = argsResult.rand;
        var shortAddrMap = result.shortAddr;
        var bytes = encodeConstructorParams(self.abi, args,shortAddrMap);
 ...
 */
var Contract = function (dece, abi, address) {
    this._dece = dece;
